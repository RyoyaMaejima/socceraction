...
def defensive_value(
    a, c, actions: DataFrame[SPADLSchema], scores: Series[float], concedes: Series[float], recoveries: Series[float], losts: Series[float], attacked: Series[float], attacks: Series[float]
) -> Series[float]:
    ...
    prev_attacked = _prev(attacked) * sameteam + _prev(attacks) * (~sameteam)

    toolong_idx = abs(actions.time_seconds - _prev(actions.time_seconds)) > _samephase_nb
    prev_recoveries[toolong_idx] = 0
    prev_attacked[toolong_idx] = 0

    prevgoal_idx = (_prev(actions.type_name).isin(['shot', 'shot_freekick', 'shot_penalty'])) & (
        _prev(actions.result_name) == 'success'
    )
    prev_recoveries[prevgoal_idx] = 0
    prev_attacked[prevgoal_idx] = 0

    prev_vdep = prev_recoveries * sameteam - c * prev_attacked * sameteam

    return (vdep - prev_vdep) * a


def value(
    a, c, actions: DataFrame[SPADLSchema], Pscores: Series[float], Pconcedes: Series[float], Precoveries: Series[float], Plosts: Series[float], Pattacked: Series[float], Pattacks: Series[float]
) -> pd.DataFrame:
    ...
    v['defensive_value'] = defensive_value(a, c, actions, Pscores, Pconcedes, Precoveries, Plosts, Pattacked, Pattacks)
